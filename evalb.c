...
calc_result(unsigned char *buf1,unsigned char *buf)
{
   ...
   if(wn1 != wn2){
    ...
    fprintf(fp, "ID =  %d\n", ID);
    ID++;
    fprintf(fp, "--- GOLD constituents ---\n");
    for(i=1;i<bn1;i++){
        char temp[MAX_SENT_LEN] = "";
        for(int j  = bracket1[i].start; j < bracket1[i].end; j++){
            strcat(temp, terminal1[j].word);
            strcat(temp, " ");
        }
        fprintf(fp, "%d (\'%s\', %d, %d, \"%s\")\n", i, bracket1[i].label, bracket1[i].start, bracket1[i].end, temp);
    }
    fprintf(fp, "# of gold = %d\n", bn1-1);
    fprintf(fp, "--- SYS constituents ---\n");
    for(i=1;i<bn2;i++){
        char temp[MAX_SENT_LEN] = "";
        for(int j  = bracket2[i].start; j < bracket2[i].end; j++){
            strcat(temp, terminal2[j].word);
            strcat(temp, " ");
        }
        fprintf(fp, "%d (\'%s\', %d, %d, \"%s\")\n", i, bracket2[i].label, bracket2[i].start, bracket2[i].end, temp);
    }
    fprintf(fp, "# of sys = %d\n", bn2-1);
    fclose(fp);
   ...
   printf("1.res=%d, 2.res=%d, 1.start=%d, 2.start=%d, 1.end=%d, 2.end=%d\n",bracket1[i].result,bracket2[j].result,bracket1[i].start,bracket2[j].start,bracket1[i].end,bracket2[j].end);
    ...
    // printf("1.label=%s, 2.label=%s, 1.start=%d, 2.start=%d, 1.end=%d, 2.end=%d\n",bracket1[i].label,bracket2[j].label,bracket1[i].start,bracket2[j].start,bracket1[i].end,bracket2[j].end);
	// does bracket match?
